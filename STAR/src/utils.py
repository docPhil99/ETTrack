...
class Trajectory_Dataloader():
            ...
            for ind, pedi in enumerate(Pedlist):
                ...
                for fi, frame in enumerate(FrameList):
                    ...
                    if self.args.dataset == 'MOT17':
                        ...
                        Trajectories.append([int(frame), current_lx, current_ly, current_w, current_h])
                    else:
                        current_rx = current_lx + current_w
                        current_ry = current_ly + current_h
                        # Add their pedID, x, y to the row of the numpy array
                        Trajectories.append([int(frame), current_lx, current_ly, current_rx, current_ry])
        ...
        for i in range(data_index.shape[1]):
            ...
            if cur_pednum >= self.args.batch_around_ped * 2:
                ...
                if batch_pednum > self.args.batch_around_ped:
                    ...
                    batch_data.append(traject_batch)
                    Batch_id.append(batch_id)
            if len(batch_data_mass) >= 20 and setname == 'train':
                break
            elif len(batch_data_mass) >= 30 and setname != 'train':
                break
    ...
    def find_trajectory_fragment(self, trajectory, startframe, seq_length):
        ...
        '''
        return_trajec = np.zeros((seq_length, 5))
...
#########################################################################################
        ...
        return_trajec[offset_start:offset_end + 1, :5] = candidate_seq
    ...
    def rotate_shift_batch(self, batch_data, ifrotate=True):
        ...
        if ifrotate:
            ...
            batch[:, :, 2] = cur_ori[:, :, 2] * np.cos(th) - cur_ori[:, :, 3] * np.sin(th)
            batch[:, :, 3] = cur_ori[:, :, 2] * np.sin(th) + cur_ori[:, :, 3] * np.cos(th)
...
def L2forTestS(outputs, targets, obs_length, lossMask, num_samples=20):
    ...
    '''
    seq_length = outputs.shape[1]
    outputs_loss = outputs - targets
    outputs_loss_ = outputs_loss.cpu().tolist()
