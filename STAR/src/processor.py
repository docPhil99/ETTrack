...
class processor(object):
    ...
    def set_optimizer(self):
        ...
        #self.smoothl1  = F.smooth_l1_loss(reduction='none')
...
########################################################################################################
            ...
            #loss_1 =F.smooth_l1_loss(outputs, batch_abs[1:9], reduction="sum")
            # loss_o = F.l1_loss(outputs, batch_offset[1:9], reduction="mean")
            lossmask, num = getLossMask(outputs, seq_list[0], seq_list[1:], using_cuda=self.args.using_cuda)
            #loss_o = torch.sum(self.criterion(outputs, batch_abs[1:9]), dim=2)
            a = F.smooth_l1_loss(outputs, batch_abs[1:9], reduction='none')
            loss_o = torch.sum(F.smooth_l1_loss(outputs, batch_abs[1:9], reduction='none'), dim=2)
            loss += (torch.sum(loss_o * lossmask / num))
            #loss_o = F.l1_loss(outputs, batch_abs[1:9], reduction="mean")
    ...
    def test_epoch(self):
        ...
        for batch in tqdm(range(self.dataloader.testbatchnums - 1)):
            ...
            loss = F.l1_loss(outputs_infer, batch_abs[1:9], reduction="mean")
            loss_epoch += loss.item()
