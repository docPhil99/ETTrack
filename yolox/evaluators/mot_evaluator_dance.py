...
from trackers.byte_tracker.tp_tracker import TPTracker
...
class MOTEvaluator:
    ...
    def evaluate(
        ...
        #tracker = byte_tp_score(net, args)
    ...
    def evaluate_sort(
        ...
        tracker = Sort_star(args, self.args.track_thresh, iou_threshold=args.iou_thresh)
        ...
        for cur_iter, (imgs, _, info_imgs, ids) in enumerate(
            ...
            # run tracking
            online_targets = tracker.update(outputs[0], info_imgs, self.img_size,net)
            online_tlwhs = []
            online_ids = []
            for t in online_targets:
                tlwh = [t[0], t[1], t[2] - t[0], t[3] - t[1]]
    ...
    def evaluate_sort_score(
        ...
        tracker = Sort_star_score(args, self.args.track_thresh, iou_threshold=args.iou_thresh)
        #tracker = Sort_star(args, self.args.track_thresh,iou_threshold=args.iou_thresh)
        ...
        for cur_iter, (imgs, _, info_imgs, ids) in enumerate(
            ...
            with torch.no_grad():
                ...
                if video_name not in video_names:
                    video_names[video_id] = video_name
                if frame_id == 1:
                    tracker = Sort_star_score(args, self.args.track_thresh, iou_threshold=self.args.iou_thresh)
                    #tracker = Sort_star(args, self.args.track_thresh)
                    if len(results) != 0:
                        result_filename = os.path.join(result_folder, '{}.txt'.format(video_names[video_id - 1]))
                        write_results_no_score(result_filename, results)
                        results = []
