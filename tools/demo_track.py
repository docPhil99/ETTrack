...
def image_demo(predictor, vis_folder, current_time, args, net):
    ...
    #tracker = Sort_star(args, det_thresh=args.track_thresh, iou_threshold=args.iou_thresh)
    #tracker = TPTracker(net, args)
    tracker = byte_tp(net, args)
    #tracker = BYTETracker_score(args)
    ...
    for frame_id, img_path in enumerate(files, 1):
        ...
        if outputs[0] is not None:
            if args.with_fastreid:
                ...
                online_targets = tracker.update(outputs[0], [img_info['height'], img_info['width']], exp.test_size) # add net
            online_tlwhs = []
            online_ids = []
...
def main(exp, args, net):
    ...
    if args.demo_type == "image":
        image_demo(predictor, vis_folder, current_time, args, net)
