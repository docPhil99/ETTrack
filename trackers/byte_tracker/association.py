...
#############################################################################################
    if (len(detections)>0 and isinstance(detections[0], np.ndarray)) or (len(trackers) > 0 and isinstance(trackers[0], np.ndarray)):
        atlbrs = detections
        btlbrs = trackers
    else:
        atlbrs = [track.tlbr for track in atracks]
        atlbrs_scores = [track.score for track in atracks]
        btlbrs = [track.tlbr for track in btracks]
    atlbrs =  np.concatenate(atlbrs, axis=0)
    atlbrs_scores = np.concatenate(atlbrs_scores, axis=0)
    btlbrs = np.concatenate(btlbrs, axis=0)
    # if len(atlbrs)>0 and len(btlbrs)>0:
    #     ct_dist1 = c_dist(atlbrs, btlbrs)
    #     ct_matrix = 1 - ct_dist1
    #     _ious = ious(atlbrs, btlbrs)
    #     cost_matrix = 1 - _ious
    #     all_matrix = cost_matrix + ct_matrix
    # else:
